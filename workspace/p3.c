...
void createQueueFromLinkedList(LinkedList *ll, Queue *q)
{
	...
	int i;								// Declare 'for' loop index i for looping

	// If the Queue q is not empty...
	if (!isEmptyQueue(q)) {
		removeAllItemsFromQueue(q);		// Remove all the items from Queue q
	}

	cur = ll->head;						// Let ListNode cur be the first ll's head (ll->head)

	// Loop through the LinkedList ll from index 0 to the size of ll
	for (i = 0; i < ll->size; i++) {
		enqueue(q, cur->item);			// Enqueue the current item into Queue q
...
{
	int i;								// Declare 'for' loop index i for looping
	int output;							// Declare the 'output' to store the dequeue's output
	int size;							// Declare the 'size' for Queue
	size = q->ll.size;					// Set the 'size' to be the size of Queue q (q->ll.size)

	// Loop from index 0 to the size of Queue q (Number of times to dequeue)
	for (i = 0; i < size; i++) {
		output = dequeue(q);			// Store the dequeue's output into 'output'

		// If output divided by 2 has a remainder of 0... 
		// *Even number*
		if (output % 2 == 0) {
			enqueue(q, output);			// Enqueue the EVEN number output into the back of Queue q
		}
	}
}
